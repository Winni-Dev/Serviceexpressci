...
export function RequestsPage() {
  ...
  const assignmentWorkerOptions = useMemo(() => {
    ...
    const options = [
      { value: 'none', label: '— Aucun travailleur —' },
      ...list.map((w) => ({
        value: w.id,
        label: w.name,
      })),
          ...
          <ScrollableDialogContent className="sm:max-w-md rounded-2xl border-0 shadow-2xl p-0 gap-0">
            ...
            <div className="p-4 pt-0 border-t border-gray-200 bg-gray-50 space-y-2 rounded-b-2xl">
              ...
              <div className="pb-1">
                ...
                <SearchableSelect
                  value={activeRequest.worker_id ?? 'none'}
                  onValueChange={(workerId) => {
                    handleStatusChange(activeRequest.id, 'assigned', workerId);
                  }}
                  options={assignmentWorkerOptions}
                  placeholder={
                    assignmentWorkerOptions.length > 1
                      ? 'Choisir un travailleur'
                      : 'Aucun travailleur dans cette zone'
                  }
                  searchPlaceholder="Tapez le nom du travailleur..."
                  emptyMessage="Aucun travailleur dans cette zone"
                  disabled={assignmentWorkerOptions.length <= 1}
                />
                {activeRequest.worker_id && (
                  <button
                    type="button"
                    onClick={() => handleUnassignWorker(activeRequest.id)}
                    className="mt-2 text-xs text-red-500 hover:text-red-700 flex items-center gap-1 transition-colors"
                  >
                    <UserX className="w-3 h-3" />
                    Désassigner ce travailleur
                  </button>
                )}
              </div>
            </div>
